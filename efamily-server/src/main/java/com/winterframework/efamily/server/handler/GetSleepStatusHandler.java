 ...
 *
 */
@Service("getSleepStatusHandler")
public class GetSleepStatusHandler extends AbstractHandler {
	@PropertyConfig("server.url.app")
	private String serverUrl;
	@PropertyConfig("app.server.get_sleep_status")
	...
	protected FmlResponse doHandle(Context ctx, FmlRequest request) throws ServerException {
		...
		Response<Integer> bizRes=http(serverUrl,urlPath,ctx,bizReqList,Integer.class);
		FmlResponse res=new FmlResponse(request);
        if(null!=bizRes){			
			res.setStatus(bizRes.getStatus().getCode()); 
			Map<String,String> responseMap = new HashMap<String,String>();
			if(bizRes.getData()!=null){
			   responseMap.put("sleepLockStatus", bizRes.getData()+"");
			}   
