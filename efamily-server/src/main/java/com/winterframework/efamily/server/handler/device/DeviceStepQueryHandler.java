 ...
 * 设备响应的计步数据入库 
 */
@Service("deviceStepQueryHandler")
public class DeviceStepQueryHandler extends AbstractHandler {
	...
	protected FmlResponse doHandle(Context ctx, FmlRequest req) throws ServerException {
		...
		log.info("20825 response time:" + DateUtils.getDate(time)+":"+data.get("data"));
