...
public class NotificationProcessorNew implements Runnable{ 
	private static final Logger log= LoggerFactory.getLogger(NotificationProcessorNew.class);
	...
	public NotificationProcessorNew(String queueName){
	...
	private void process(){
		synchronized(queueName){	//防止并发 必须要加锁
			...
			if(isConnected){
				...
				String dataStr=redisQueue.get(offlineQueueName);
				while(null!=dataStr){
					notyTask=JsonUtils.fromJson(dataStr, NotyTask.class);
					doProcess(notyTask,true);
					boolean isConnected2=ChannelManager.isConnected(notyTask.getTarget().getUserId(), notyTask.getTarget().getDeviceId());
					if(!isConnected2){
						break;	//离线队列中的推送 一旦离线就可中断推送
					}
					dataStr=redisQueue.get(offlineQueueName);
						
				}
			}
			String dataStr=redisQueue.get(queueName); 
			while(null!=dataStr){
				notyTask=JsonUtils.fromJson(dataStr, NotyTask.class);
				doProcess(notyTask,false);
				dataStr=redisQueue.get(queueName);
			}
		}
	}
	private void doProcess(NotyTask notyTask,boolean isOfflineQueue) {
		...
		if(!isConnected && rule.isOffline() && (expireTime==-1 || ((DateUtils.getCurTime()-taskTime)/1000/60<=expireTime))){
			...
			if(null!=deviceId){
				...
				addQueue(notyTask,true,isOfflineQueue);//加入离线推送队列
			}else{
		...
		if(isConnected){
			...
			try {
				...
				addQueue(notyTask,true,isOfflineQueue);
			}
