...
public final class SqlSessionUtils2 {
	private static final Logger logger = LoggerFactory.getLogger(SqlSessionUtils2.class);
	...
	public static SqlSession getSqlSession(SqlSessionFactory sessionFactory, DataSource dataSource,
		...
		Object o= TransactionSynchronizationManager.getResource(sessionFactory);
		if(o instanceof DefaultSqlSession){
			return (DefaultSqlSession)o;
		}
	...
	public static void commitSqlSession(SqlSession session, SqlSessionFactory sessionFactory, boolean force)
		...
		if(o instanceof SqlSessionHolder){
			holder = (SqlSessionHolder) TransactionSynchronizationManager.getResource(sessionFactory);
		}
		if (holder == null) {
	...
	public static void rollbackSqlSession(SqlSession session, SqlSessionFactory sessionFactory, boolean force)
		...
		if(o instanceof SqlSessionHolder){
			holder = (SqlSessionHolder) TransactionSynchronizationManager.getResource(sessionFactory);
		}else if(o instanceof DefaultSqlSession){
			((DefaultSqlSession)o).commit();
		}
	...
	public static boolean usesManagedTransactions(Configuration configuration) {
		return configuration.getEnvironment().getTransactionFactory().getClass()
				.isAssignableFrom(ManagedTransactionFactory.class);
	}
		...
		public void afterCompletion(int status) {
			try {
				...
				if (!holder.isOpen()) {
					...
					if (manageConnection) {
						logger.debug("Transaction synchronization closing JDBC Connection");
