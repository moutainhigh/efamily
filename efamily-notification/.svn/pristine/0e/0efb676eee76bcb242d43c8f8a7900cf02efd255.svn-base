...
public class NotificationServiceImpl implements INotificationService{
	...
	public Long notify(Context ctx,Notification notification) throws NotificationException{
		...
		if(!notification.isRealTime()){
			...
			task.setId(taskId);
			task.setOfflineTitle(message.getOfflineTitle());
			task.setOfflineData(message.getOfflineData());
	...
	private void notifyRealTime(Long userId,Long deviceId,Long taskId,int command,Map<String,String> data){ 
		...
		try{
			...
			if(pushStatus.equals(PushStatus.SUCCESS)){
				log.info("notify real time success.userId="+userId+" deviceId="+deviceId+" command="+command);
			}else if(pushStatus.equals(PushStatus.OFFLINE)){
				log.info("notify real time offline.userId="+userId+" deviceId="+deviceId+" command="+command);
			}else{
				log.info("notify real time failed.userId="+userId+" deviceId="+deviceId+" command="+command);
			}
		}catch(Exception e){
