...
public class  NotificationOfflineProcessorNew2 implements Runnable{ 
	...
	private final ICcontrolService ccontrolService=SpringContextHolder.getBean("ccontrolServiceImpl");
	...
	private void process(){ 
		...
		try{
			boolean isConnected=ccontrolService.isConnected(target.getUserId(), target.getDeviceId());
			if(isConnected){
			...
			log.error("is connected error. userId="+target.getUserId()+" deviceId="+target.getDeviceId(), e);
		}
	}
